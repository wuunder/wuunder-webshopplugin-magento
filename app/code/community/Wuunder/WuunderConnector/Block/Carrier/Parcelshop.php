 ...
 */
class Wuunder_WuunderConnector_Block_Carrier_Parcelshop extends Mage_Core_Block_Template
{
    /**
    ...
    {
        if ($bool && !Mage::getStoreConfig('carriers/wuunderdpd/google_maps_display'))
            $this->_showurl = $bool;
        return;
    }
    ...
    public function getConfig()
    {
        ...
        $this->_configArray["ParcelUrl"] = $this->getUrl('wuunderconnector/ajax/index', array('_secure' => true));
        ...
        $this->_configArray["gmapsCustomIcon"] = (Mage::getStoreConfig('carriers/wuunderdpd/google_maps_icon') ? Mage::getBaseUrl('media') . "dpd/" . Mage::getStoreConfig('carriers/dpdparcelshops/google_maps_icon') : "");
        $this->_configArray["gmapsDisplay"] = (bool)Mage::getStoreConfig('carriers/wuunderdpd/google_maps_display');
        $this->_configArray["loaderimage"] = $this->getSkinUrl('images/dpd/ajax-loader.gif');
        $this->_configArray["freeShippingOnCustom"] = (bool)Mage::getStoreConfig('carriers/wuunderdpd/custom_parcelshops_free_shipping');
    ...
    public function getShopsHtml()
    {
        ...
        foreach ($this->getParcelShops()->parcelShop as $shop) {
            ...
            $html .= '<strong>' . Mage::helper('wuunderconnector')->__('Ship to this ParcelShop.') . '</strong>';
            $html .= '</a>';
            $html .= '</div>';
...
//        }
    ...
    public function getOpeningHoursHtml($dpdExtraInfo)
    {
        ...
        if (is_array(json_decode($dpdExtraInfo))) {
            foreach (json_decode($dpdExtraInfo) as $hours) {
                ...
                $html .= '<li class="day">' . Mage::helper('wuunderconnector')->__($hours->weekday) . '</li>';
                if ($hours->openMorning != "") {
                    $html .= '<li class="hour">' . $hours->openMorning . ' - ' . $hours->closeMorning . '</li>';
                } else {
                    $html .= '<li class="hour">' . Mage::helper('wuunderconnector')->__('closed') . '</li>';
                }
    ...
    protected function _getSpecialShopsHtml($html, $counter)
    {
        ...
        $specialShopsCollection = Mage::getModel('wuunderconnector/specialparcelshops')->getCollection()->addFieldToFilter('parcelshop_country', array('eq' => $countryCode))->addFieldToFilter('parcelshop_website_id', array('eq' => $websiteId));

        foreach ($specialShopsCollection as $specialShop) {
            $html .= '<div class="shop-data" id="shop' . $specialShop->getParcelshopDelicomId() . '">';
            if (Mage::getStoreConfig('carriers/wuunderdpd/special_parcelshop_icon') != "") {
                $html .= '<img class="specialparcelshopImage" height="50" width="50" src="' . Mage::getBaseUrl('media') . "dpd/" . Mage::getStoreConfig('carriers/wuunderdpd/special_parcelshop_icon') . '"/>';
            }
            ...
            $this->_configArray['shop' . $specialShop->getParcelshopDelicomId()]['specialImage'] = (Mage::getStoreConfig('carriers/wuunderdpd/special_parcelshop_icon') != "" ? Mage::getBaseUrl('media') . "dpd/" . Mage::getStoreConfig('carriers/dpdparcelshops/special_parcelshop_icon') : "");
    ...
    {
        if ($special && Mage::getStoreConfig('carriers/wuunderdpd/special_parcelshop_icon')) {
            $image = Mage::getBaseUrl('media') . "dpd/" . Mage::getStoreConfig('carriers/wuunderdpd/special_parcelshop_icon');
        } else {
                    ...
                    <table>
                        <tbody>
                            <tr class="pointer">
                                ...
                                <td id="' . 'shop' . ($special ? $shop->getParcelshopDelicomId() : $shop->parcelShopId) . '" class="parcelshoplink" onclick="window.dpdShipping.saveParcelShop(event);"><strong>' . Mage::helper('wuunderconnector')->__('Ship to this ParcelShop.') . '</strong></td>
                            </tr>
                        </tbody>
                    </table>
