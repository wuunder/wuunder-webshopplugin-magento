...
/**
 * Class Wuunder_Wuunderconnector_Model_Dpdwebservice
 */
class Wuunder_Wuunderconnector_Model_Dpdwebservice extends Mage_Core_Model_Abstract
{
...
//    CONST WEBSERVICE_PARCELSHOP = 'ParcelShopFinderService/V3_0/?wsdl';
     ...
     */
    CONST XML_PATH_DPD_URL = 'wuunderconnector/connect/dpd_url';

    /**
     * XML path to configuration setting for userid.
     */
    CONST XML_PATH_DPD_USERID = 'wuunderconnector/connect/dpd_user_id';

    /**
     * XML path to configuration setting for password.
     */
    CONST XML_PATH_DPD_PASSWORD = 'wuunderconnector/connect/dpd_password';

    /**
     * XML path to configuration setting for sender name.
     */
    CONST XML_PATH_DPD_SENDER_NAME = 'wuunderconnector/connect/company';

    /**
     * XML path to configuration setting for sender street.
     */
    CONST XML_PATH_DPD_SENDER_STREET = 'wuunderconnector/connect/streetname';

    /**
     * XML path to configuration setting for sender streetnumber.
     */
    CONST XML_PATH_DPD_SENDER_STREETNUMBER = 'wuunderconnector/connect/housenumber';

    /**
     * XML path to configuration setting for sender country.
     */
    CONST XML_PATH_DPD_SENDER_COUNTRY = 'wuunderconnector/connect/country';

    /**
     * XML path to configuration setting for sender zipcode.
     */
    CONST XML_PATH_DPD_SENDER_ZIPCODE = 'wuunderconnector/connect/zipcode';

    /**
     * XML path to configuration setting for sender city.
     */
    CONST XML_PATH_DPD_SENDER_CITY = 'wuunderconnector/connect/city';
    ...
    protected function _login()
    {
        ...
        $password = Mage::getStoreConfig(self::XML_PATH_DPD_PASSWORD);
        ...
        try {
            ...
            Mage::helper('wuunderconnector')->log('Login for webservice succeeded', Zend_Log::INFO);
            Mage::helper('wuunderconnector')->log($result, Zend_Log::DEBUG);
        } catch (SoapFault $soapE) {
            Mage::helper('wuunderconnector')->log('Webservice Login failed:', Zend_Log::ERR);
            Mage::helper('wuunderconnector')->log($soapE, Zend_Log::ERR);
            ...
            Mage::helper('wuunderconnector')->log($e->getMessage(), Zend_Log::ERR);
            return false;
        }
        ...
        while (!$stop && $count++ < self::MAX_LOGIN_RETRY) {
            try {
                ...
                if (isset($result->orderResult->shipmentResponses->faults) || isset($result->orderResult->shipmentResponses->faultString)) {
                    Mage::helper('wuunderconnector')->log('Webservice ' . $method . ' failed:', Zend_Log::ERR);
                    if (isset($result->orderResult->shipmentResponses->faults)) {
                        Mage::helper('wuunderconnector')->log($result->orderResult->shipmentResponses->faults, Zend_Log::ERR);
                    }
                    if (isset($result->orderResult->shipmentResponses->faultString)) {
                        Mage::helper('wuunderconnector')->log($result->orderResult->shipmentResponses->faultString, Zend_Log::ERR);
                    }
                    return false;
                }


                Mage::helper('wuunderconnector')->log('Webservice ' . $method . ' succeeded', Zend_Log::INFO);
//                Mage::helper('wuunderconnector')->log($result, Zend_Log::DEBUG);

            } catch (SoapFault $soapE) {
                if (isset($soapE->detail) && isset($soapE->detail->authenticationFault)) {
                    if ($soapE->detail->authenticationFault->errorCode == 'LOGIN_5') {
                        Mage::helper('wuunderconnector')->log('Athentication token expired, retrying...', Zend_Log::INFO);
                        $this->_login();
                    } else {
                        Mage::helper('wuunderconnector')->log('Webservice ' . $method . ' failed:', Zend_Log::ERR);
                        Mage::helper('wuunderconnector')->log($soapE->detail, Zend_Log::ERR);
                        Mage::getSingleton('adminhtml/session')->addError($soapE->detail->authenticationFault->errorMessage);
                        return false;
                    }
                } else {
                    Mage::helper('wuunderconnector')->log($soapE->getMessage(), Zend_Log::ERR);
        ...
        if ($stop == false) {
            Mage::helper('wuunderconnector')->log('Athentication went wrong!', Zend_Log::ERR);
            return false;
        } else {
            return $result;
        }
    ...
    public function getShippingLabel($recipient, Mage_Sales_Model_Order $order, $shipment, $parcelshop = false)
    {
        ...
        $language = Mage::helper('wuunderconnector')->getLanguageFromStore($order->getStoreId());
